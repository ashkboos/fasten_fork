...
public class RocksDao implements Closeable {
    ...
    private GraphMetadata.ReceiverRecord.CallType transformCallType(eu.fasten.core.data.metadatadb.codegen.enums.ReceiverType type) {
        switch (type) {
            case dynamic:
    ...
    public void saveToRocksDb(final GidGraph gidGraph) throws IOException, RocksDBException {
        ...
        if (gidGraph instanceof ExtendedGidGraph) {
            ...
            final Map<Pair<Long, Long>, EdgesRecord> edgesInfo = extendedGidGraph.getCallsInfo();
            ...
            edgesInfo.forEach((pair, record) -> map.compute(pair.getFirst().longValue(), (k, list) -> {
                ...
                var line = record.component3()[0].getLine();
                var callType = transformCallType(record.component3()[0].getType());
                var uri = gidToUriMap.get(pair.getFirst());
                var types = new ArrayList<String>();
                for (var receiver : record.component3()) {
                    types.add(receiver.getReceiverUri());
                }
                list.add(new ReceiverRecord(line, callType, uri, types));
                return list;
            }));
